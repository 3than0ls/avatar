...
export default function ImageModal() {
  ...
  return (
    <div className="fixed text-center w-full h-screen bg-gray-800 bg-opacity-75 flex justify-center overflow-hidden items-center z-30">
      <div className="relative w-full overflow-y-auto max-w-3xl h-auto max-h-full bg-white rounded-lg border-gray-200 border-2 p-6">
        ...
        <div className="flex justify-center">
          {editMode ? (
            <AvatarEditor src={imageUrl} className="mb-3 mx-auto" name={name} ref={editorRef} />
          ) : (
            <img className="w-96 h-96 object-cover" src={imageUrl} alt={name} />
          )}
        </div>
